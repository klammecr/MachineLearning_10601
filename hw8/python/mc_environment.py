...
from mc_tiles import tiles, IHT
...
# Environment
class MountainCar:
    def __init__(self, mode=None, fixed = None):
        ...
        self.action_space = 3
        self.act_to_idx = {0: 0, 1: 1, 2: 2} # mapping from actions to indices in the weight matrix
        self.idx_to_act = {0: 0, 1: 1, 2: 2}
        ...
        self.mode = mode
        # used to make reset random
        if (fixed == 1): self.random = False
        else: self.random = True
    ...
    def reset(self):
        ...
        else: self.state = np.array([self.np_random.uniform(low=-0.6, high=-0.4), 0])
    ...
    def render(self, mode='human'):
        ...
        if self.viewer is None:
            import mc_rendering as rendering
